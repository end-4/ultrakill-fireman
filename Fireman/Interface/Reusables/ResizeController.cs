...
public class ResizeController : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler, IPointerDownHandler,
    ...
    private Texture2D GetTexture(string name) {
        return ThornClient.Managers.AssetManager.Get<Texture2D>(Plugin.BundleKey, name)!;
    }
    ...
    private void UpdateHoverState() {
        ...
        // // Inner
        // bool onTop = localMousePos.y >= rect.yMax - grabAreaThickness && localMousePos.y <= rect.yMax;
        // bool onBottom = localMousePos.y <= rect.yMin + grabAreaThickness && localMousePos.y >= rect.yMin;
        // bool onLeft = localMousePos.x <= rect.xMin + grabAreaThickness && localMousePos.x >= rect.xMin;
        // bool onRight = localMousePos.x >= rect.xMax - grabAreaThickness && localMousePos.x <= rect.xMax;
    ...
    public void OnDrag(PointerEventData eventData) {
        ...
        _rectTransform.localPosition = (Vector3)_originalLocalPosition + new Vector3(positionOffset.x, positionOffset.y, 0f);
    }
